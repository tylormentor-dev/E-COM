...
CREATE TABLE `mechanic_connect`.`users` (
 ...
 `password_hash` VARCHAR(255) NOT NULL,
  ...
  CREATE TABLE `mechanic_connect`.`mechanics` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `fk_mechanics_user`
    FOREIGN KEY (`user_id`)
    REFERENCES `mechanic_connect`.`users` (`id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION);
    
    
CREATE TABLE `mechanic_connect`.`services` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  INDEX `fk_mechanic_services_idx` (`mechanic_id` ASC) VISIBLE,
  CONSTRAINT `fk_mechanic_services`
    FOREIGN KEY (`mechanic_id`)
    REFERENCES `mechanic_connect`.`mechanics` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE);
    
    
    
CREATE TABLE `mechanic_connect`.`bookings` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `mechanic_id` INT NOT NULL,
  ...
  CONSTRAINT `fk_bookings_service`
    ...
    REFERENCES `mechanic_connect`.`services` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE);
    
    
CREATE TABLE `mechanic_connect`.`car_listing` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  INDEX `fk_carlisting_idx` (`seller_id` ASC) VISIBLE,
  CONSTRAINT `fk_carlisting`
    ...
    CREATE TABLE `mechanic_connect`.`car_images` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `car_id` INT NOT NULL,
  ...
  CONSTRAINT `fk_car_id`
    FOREIGN KEY (`car_id`)
    REFERENCES `mechanic_connect`.`car_listing` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE);
    

    CREATE TABLE `mechanic_connect`.`car_parts` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(100) NOT NULL,
...
CREATE TABLE `mechanic_connect`.`orders` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `user_id` INT NOT NULL,
  `total_price` DECIMAL(10,2) NOT NULL,
...
CREATE TABLE mechanic_connect.order_items (
  ...
  CONSTRAINT chk_product CHECK (
    (car_id IS NOT NULL AND part_id IS NULL)
    OR
    (car_id IS NULL AND part_id IS NOT NULL)
  )
);

    
    
CREATE TABLE `mechanic_connect`.`reviews` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `user_id` INT NOT NULL,
  `mechanic_id` INT NOT NULL,
...
CREATE TABLE `mechanic_connect`.`notifications` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `user_id` INT NOT NULL,
...
CREATE TABLE `mechanic_connect`.`payments` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `user_id` INT NOT NULL,
  `order_id` INT NULL,
  ...
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  INDEX `fk_payment_id_idx` (`user_id` ASC) VISIBLE,
  INDEX `fk_payment_order_idx` (`order_id` ASC) VISIBLE,
  UNIQUE INDEX `transaction_ref_UNIQUE` (`transaction_ref` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_payment_order`
    FOREIGN KEY (`order_id`)
    REFERENCES `mechanic_connect`.`orders` (`id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE);
...
ALTER TABLE reviews
ADD UNIQUE KEY unique_review (user_id, mechanic_id) VISIBLE;	

ALTER TABLE `mechanic_connect`.`orders` 
CHANGE COLUMN `status` `status` ENUM('pending', 'paid', 'shipped', 'delivered', 'cancelled') NOT NULL DEFAULT 'pending' ;

ALTER TABLE bookings
ADD UNIQUE KEY unique_booking (mechanic_id, booking_date);


ALTER TABLE `mechanic_connect`.`orders` 
CHANGE COLUMN `payment_method` `payment_method` ENUM('card', 'cash', 'eft', 'wallet') NULL DEFAULT NULL ;
